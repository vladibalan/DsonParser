...
Laura for Genesis 9.duf  -> scene.modifiers (3 entries):
  ...
  - Laura_figure_ctrl_Character cv=1 -> CONTROL: 0 deltas, 2 formulas
        +- output ...Laura_head_bs_Head.dsf#Laura_head_bs_Head?value  (x dial)
        |     \- Laura_head_bs_Head.dsf : 0 deltas, 44 formulas -> leaf morphs (deltas)
        \- output ...Laura_body_bs_body.dsf#Laura_body_bs_body?value  (x dial)
...
### Scene-node authored channels (`scene.nodes[].extra[].studio_node_channels`) - [DONE] implemented (Aug 2026)
...
### Polyline list - strand/curve geometry (`geometry_library[].polyline_list`) - [DONE] implemented (Aug 2026)
...
verts `96..191`; Sum(per-polyline vertex counts) = `188448`, which minus `1963` =
`186485`.

**Consumer note (additive, non-breaking):** the six new functions are available to
the UE plugin; existing calls are unaffected.

### Scene animations full-keyframe surface (count + per-key time + numeric value) - [DONE] implemented (Jul 2026)
...
### Gzip blank (all-zero) trailer acceptance - DAZ-compat loader - [DONE] implemented (Jun 2026)
...
### Channel value faithfulness - bool coercion (2.2.1) + type-mismatch audit (2.2.2) - [DONE] implemented (Jun 2026)
...
### Modifier control-inventory metadata - group / region / icon - [DONE] implemented (Jun 2026)
...
### Asset-catalog metadata - presentation + geograft - [DONE] implemented (Jun 2026)
...
### Geograft weld correspondence (`graft.vertex_pairs` / `hidden_polys`) - [DONE] implemented (Jul 2026)
...
### Geometry rigidity (`geometry.rigidity`) - [DONE] implemented (Jul 2026)
...
### Per-layer LIE compositing metadata (`Image::layers`) - [DONE] implemented (Jun 2026)
...
### Scene animations key-0 channel surface (`scene.animations`) - [DONE] implemented (Jun 2026)
...
### Scene post-load addon manifest (`scene.extra` Character Addon Loader) - [DONE] implemented (Jun 2026)
...
### Scene post-load scripts (`scene.extra` DAZ Script references) - [DONE] implemented (Jun 2026)
...
### Image `map_size` (pixel dimensions) - [DONE] implemented (Jun 2026)
...
in `DsonTest2.cpp` (`HID_Nancy_9.duf`, expects 4096x4096 plus the `0` out-of-range
sentinel).
