# DsonParser Changelog - v1.x (sealed volume)

Sealed archive of the v1.x capability epoch (1.0.0 - 1.6.0), rotated verbatim out of the root
`CHANGELOG.md` on 2026-08-08 when it crossed the 64 KB doc ceiling (`Docs/DocRules.md`,
"Cold-doc rotation, in detail"). Immutable once sealed: entries stand exactly as released; new
history lands in the root `CHANGELOG.md`. Cite by root + date via the root's per-entry index -
never by this volume file.
...
28 new accessors = 14 shared suffixes x 2 prefixes (per-image
